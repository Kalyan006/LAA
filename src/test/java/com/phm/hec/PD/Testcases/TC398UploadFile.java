...
import com.phm.hec.CM.Objects.FormResourceAndEducationObjects;
...
public class TC398UploadFile {
	...
	public static void FileUploadPreCondition() throws Throwable {
		...
		try {
			...
			Point a = RPO.ClkCaremgtchkbox.getLocation();
			logger.info(a);
			if (RPO.ClkCaremgtchkbox.isSelected()) {	
				
				if(RPO.ClkCaremgtchkbox.getLocation()==a){
				if (RPO.ClkformsEdusubmenuP9.isSelected()) {
					...
					TC_398_UploadFile.log(LogStatus.INFO, "Enabling the Forms Resource And Education Feature");
						RPO.ClickFormResourceEducationSubMenuP9();
		
					
				}
			}else{
				if (RPO.ClkformEdusubmenuP1.isSelected()) {
					...
					logger.info("Enabling the Forms Resource And Education Feature...");
					TC_398_UploadFile.log(LogStatus.INFO, "Enabling the Forms Resource And Education Feature");
						RPO.ClickFormResourceEducationSubMenuP1();
	...
	public static void UploadFile() throws Throwable {
		...
		try {
			...
			TC_398_UploadFile.log(LogStatus.PASS, "Logout from the PEC application.");
			Thread.sleep(GlobalVar.threadSleep);
			HPO.FREPageLogout();

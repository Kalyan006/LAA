...
import com.phm.hec.BM.Objects.BMPageObject;
		...
		try {
...

			{
				...
				if (Execute == true) {
					...
					String BMProviderNPI = BMP.StoreProviderNPI();
					logger.info(BMProviderNPI);
					
					String BMProviderTIN = BMP.StoreProviderTIN();
					logger.info(BMProviderTIN);
					
					Thread.sleep(GlobalVar.threadSleep);
					logger.info("PHM ICON Clicked");
					HPO.BMClickPHM();
					logger.info("PHM ICON Clicked");
